...
const usersDir = path_1.default.join(__dirname, 'users');
if (!fs_1.default.existsSync(usersDir)) {
    fs_1.default.mkdirSync(usersDir, { recursive: true });
}
// Create transactions hub directory
const transactionsHubDir = path_1.default.join(__dirname, 'transactions_hub');
if (!fs_1.default.existsSync(transactionsHubDir)) {
    fs_1.default.mkdirSync(transactionsHubDir, { recursive: true });
}
// Create PDF attachments directory
const pdfAttachmentsDir = path_1.default.join(__dirname, 'pdf_attachments');
if (!fs_1.default.existsSync(pdfAttachmentsDir)) {
    fs_1.default.mkdirSync(pdfAttachmentsDir, { recursive: true });
}
...
app.post('/api/fetch-emails', async (req, res) => {
    ...
    try {
        ...
        // Save to JSON file
        fs_1.default.writeFileSync(filepath, JSON.stringify(storageData, null, 2));
...
app.post('/api/fetch-emails-enhanced', async (req, res) => {
        ...
        try {
            ...
            if (userId) {
                ...
                if (fs_1.default.existsSync(userFilePath)) {
                    const userData = fs_1.default.readFileSync(userFilePath, 'utf8');
                    ...
                    const geminiFilepath = path_1.default.join(transactionsHubDir, geminiFilename);
                    fs_1.default.writeFileSync(geminiFilepath, JSON.stringify(geminiData, null, 2));
...
app.post('/api/create-transactions-hub', async (req, res) => {
    ...
    try {
        ...
        const files = fs_1.default.readdirSync(dataDir)
            ...
            .sort((a, b) => {
            const statsA = fs_1.default.statSync(path_1.default.join(dataDir, a));
            const statsB = fs_1.default.statSync(path_1.default.join(dataDir, b));
            return statsB.mtime.getTime() - statsA.mtime.getTime();
        });
        // Load Gemini processed data files
        const geminiFiles = fs_1.default.readdirSync(transactionsHubDir)
            .filter(file => file.startsWith('gemini_transactions_') && file.endsWith('.json'))
            .sort((a, b) => {
            const statsA = fs_1.default.statSync(path_1.default.join(transactionsHubDir, a));
            const statsB = fs_1.default.statSync(path_1.default.join(transactionsHubDir, b));
            return statsB.mtime.getTime() - statsA.mtime.getTime();
        });
        const allTransactions = [];
        // First, load Gemini processed transactions (preferred)
        for (const file of geminiFiles) {
            const filepath = path_1.default.join(transactionsHubDir, file);
        ...
        const filepath = path_1.default.join(transactionsHubDir, filename);
        // Save to JSON file
        fs_1.default.writeFileSync(filepath, JSON.stringify(transactionsHubData, null, 2));
...
// Read PDF statement
app.post('/api/read-pdf-statement', async (req, res) => {
...
app.get('/api/transactions-hub', (req, res) => {
    console.log('[API] transactions-hub: Fetching transactions hub files');
    try {
        const files = fs_1.default.readdirSync(transactionsHubDir)
            .filter(file => file.endsWith('.json'))
            .map(file => {
            const filepath = path_1.default.join(transactionsHubDir, file);
...
app.get('/api/transactions-hub/:filename', (req, res) => {
...
app.get('/api/pdf-attachments/:filename', (req, res) => {
    const { filename } = req.params;
    console.log(`[API] pdf-attachments: Serving PDF ${filename}`);
    try {
        const filepath = path_1.default.join(pdfAttachmentsDir, filename);
        if (!fs_1.default.existsSync(filepath)) {
            return res.status(404).json({
                success: false,
...
function getPDFPasswordInfo(filename) {
    try {
        ...
        const transactionsDir = path_1.default.join(__dirname, 'transactions_hub');
        if (!fs_1.default.existsSync(transactionsDir)) {
            return { password: null, isProtected: false };
        }
        const geminiFiles = fs_1.default.readdirSync(transactionsDir)
            .filter(file => file.startsWith('gemini_') && file.endsWith('.json'));
        for (const geminiFile of geminiFiles) {
            const filepath = path_1.default.join(transactionsDir, geminiFile);
...
app.get('/api/pdf-attachments', (req, res) => {
    console.log('[API] pdf-attachments: Fetching list of stored PDFs');
    try {
        const files = fs_1.default.readdirSync(pdfAttachmentsDir)
            .filter(file => file.endsWith('.pdf'))
            .map(file => {
            const filepath = path_1.default.join(pdfAttachmentsDir, file);
            const stats = fs_1.default.statSync(filepath);
...
app.post('/api/debug-pdf-detection', async (req, res) => {
...
app.post('/api/setup-user', (req, res) => {
    ...
    try {
        ...
        const userFilePath = path_1.default.join(usersDir, `${userId}.json`);
        fs_1.default.writeFileSync(userFilePath, JSON.stringify(userProfile, null, 2));
...
function extractFullEmailBody(payload) {
    if (!payload)
        return '';
    let emailBody = '';
...
function extractBankDetails(snippet) {
    ...
    const foundKeywords = bankKeywords.filter(keyword => snippet.toLowerCase().includes(keyword));
...
function buildSearchQueries(queryType, userProfile) {
    const queries = [];
...
function mapAccountToEmail(emailInfo, userProfile) {
    const mapping = {
        bankAccount: null,
        creditCard: null,
        confidence: 0
...
function extractTransactionDetails(snippet, payload) {
    if (!snippet)
        return null;
...
function extractStatementDetails(snippet, payload, userProfile) {
    if (!snippet)
        return null;
    const content = snippet.toLowerCase();
    const details = {
...
app.get('/api/user-profile/:userId', (req, res) => {
    ...
    try {
        ...
        const fileContent = fs_1.default.readFileSync(userFilePath, 'utf8');
...
app.get('/api/user-profiles', (req, res) => {
    ...
    try {
        const files = fs_1.default.readdirSync(usersDir)
            .filter(file => file.endsWith('.json'))
            .map(file => {
            const filepath = path_1.default.join(usersDir, file);
            const stats = fs_1.default.statSync(filepath);
            const fileContent = fs_1.default.readFileSync(filepath, 'utf8');
...
app.put('/api/user-profile/:userId', (req, res) => {
    ...
    try {
        ...
        fs_1.default.writeFileSync(userFilePath, JSON.stringify(updatedProfile, null, 2));
...
function buildEnhancedSearchQueries(emailType, fromDate, toDate) {
    const queries = [];
    let dateFilter = '';
    ...
    if (fromDate || toDate) {
        const dateParts = [];
        if (fromDate)
            dateParts.push(`after:${fromDate.split('T')[0]}`);
        if (toDate)
            dateParts.push(`before:${toDate.split('T')[0]}`);
...
function extractEnhancedTransactionDetails(snippet, payload) {
    if (!snippet)
        return null;
    const content = snippet.toLowerCase();
    const details = {
...
function extractEnhancedStatementDetails(snippet, payload) {
    if (!snippet)
        return null;
    const content = snippet.toLowerCase();
    const details = {
...
async function downloadAndStorePDF(gmail, attachmentId, emailId, filename) {
    try {
        ...
        const filepath = path_1.default.join(pdfAttachmentsDir, uniqueFilename);
        // Save the PDF file
...
function extractTransactionsFromEmail(email, userProfile) {
    const transactions = [];
...
async function extractAndReadPDF(messageData, password) {
    try {
        // Find PDF attachment
...
app.post('/api/process-transactions-gemini', async (req, res) => {
...
async function processEmailsWithGemini(emails, userProfile) {
...
function createBatchGeminiPrompt(emails, userProfile) {
...
${email.pdfAttachments && email.pdfAttachments.length > 0 ? `- PDF Files: ${email.pdfAttachments.map((pdf) => pdf.originalFilename || pdf.storedFilename || pdf.filename || pdf.name || 'Unknown').join(', ')}` : ''}
`).join('\n');
    const prompt = `
...
function createGeminiPrompt(email, userProfile) {
    const prompt = `
...
function extractBalanceFromEmails(emails, userProfile) {
...
function extractBalanceFromGeminiData(transactions, fallbackBalance) {
...
function parseBatchGeminiResponse(response, emails) {
    try {
...
function parseGeminiResponse(response, email) {
    try {
        // Clean the response to extract JSON
        const jsonMatch = response.match(/\[[\s\S]*\]/);
        if (!jsonMatch) {
...
app.delete('/api/delete-all-data', async (req, res) => {
    ...
    try {
        ...
        if (fs_1.default.existsSync(pdfAttachmentsDir)) {
            const pdfFiles = fs_1.default.readdirSync(pdfAttachmentsDir);
        ...
        if (fs_1.default.existsSync(transactionsHubDir)) {
            const transactionFiles = fs_1.default.readdirSync(transactionsHubDir);
...
app.delete('/api/delete-file/:type/:filename', async (req, res) => {
    ...
    try {
        ...
        switch (type) {
            case 'pdf':
                filepath = path_1.default.join(pdfAttachmentsDir, filename);
                break;
            case 'data':
                filepath = path_1.default.join(dataDir, filename);
                break;
            case 'transaction':
                filepath = path_1.default.join(transactionsHubDir, filename);
                break;
            default:
                return res.status(400).json({
                    success: false,
